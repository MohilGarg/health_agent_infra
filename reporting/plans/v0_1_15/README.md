...
- **Phase 0 / D11 bug-hunt (2026-05-03 evening):** internal sweep + audit-chain probe + persona matrix (13/13 clean). Surfaced **F-PHASE0-01 (revises-scope)** — W-C `nutrition_target` table proposal duplicates the existing `target` table (migration 020, in tree since v0.1.8 W50). Plus 3 nits. Maintainer chose F-PHASE0-01 Option A (extend existing table). PLAN §2.B/§2.D/§2.E/§4/§5 + cross-doc fan-out applied. Q2 (foreign-user candidate) closed: Mohil Garg (Imperial Econ Data Sci & Finance Y1), path (a) cycle proceeds.
